...
df_anomalies_global = pd.DataFrame()
...
for context in range(1 , len(df_tw) + 1):
    ...
    for num_cluster in range(1, max_cluster + 1):
        ...
        df_anomalies_global = pd.concat([df_anomalies_global, anm_table_overall], ignore_index=True)

df_anomalies_global.to_csv(f"data/diagnosis/anomalies_table_var/final_anomalies_{var}.csv", index=False)
