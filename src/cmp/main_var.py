...
time_windows = pd.read_csv("data/time_windows_new.csv")
group_cluster = pd.read_csv("data/group_cluster_new.csv", index_col="timestamp")

# Lista di variabili
el_uta_list = ["el_UTA_3_3B_7"]

# Dizionario in cui verranno salvati i dataframe di anomalie per ogni combinazione di context e num_cluster
anomalies_dict = {}

# Ciclo esterno: iterazione sui contesti (da 1 al numero di righe di time_windows)
for context in range(1, len(time_windows) + 1):

    # Usa context-1 per accedere correttamente a df_tw (che è 0-based)
    ...
    df_time_window = pd.DataFrame({
        'id': 1,
        'description': [f"[{to_tw.strftime('%H:%M')} - {from_tw.strftime('%H:%M')})"],
        'observations': [int((to_tw_float - from_tw_float) * 4)],
        'from': from_tw.strftime("%H:%M"),
        'to': to_tw.strftime("%H:%M"),
        'duration': [
            f"{int((to_tw_float - from_tw_float) * 3600)}s (~{(to_tw_float - from_tw_float):.2f} hours)"]
    })
    ...
    # Per il numero di cluster, qui si assume di usare il numero di cluster univoci presenti in df_cluster['cluster'].
    ...
    for num_cluster in range(1, max_cluster + 1):
        ...
        anm_table_overall = pd.DataFrame(columns=["date"])

        for var in el_uta_list:
            aule_list = var.split("_")[2:]
            t_ext_done = False

            for aula in aule_list:
                # Lista delle variabili interne da analizzare
                var_list = [f"T_amb_{aula}", f"T_setpoint_{aula}", "t_ext"]


                # Seleziona le date di anomalie dal dataframe globale in base a context e num_cluster
                anomalies_dates = df_anm.loc[
                    (df_anm['Cluster'] == num_cluster) & (df_anm['Context'] == context), 'timestamp'
                ].tolist()
                anomalies_dates = [pd.to_datetime(ts).date() for ts in anomalies_dates]

                # Preprocessing dei dati per ogni aula
                var_dict_df = preprocess_variabili_interne(var)
                # Seleziona le date univoche per il cluster corrente (si assume che la colonna 'cluster' contenga stringhe tipo "Cluster_1", "Cluster_2", ...)
                ...
                for var_int in var_list:
                    ...
                    df_count = data.groupby('date').size().reset_index(name='n_righe')
                    ...
                    # Aggiorna la tabella overall con le anomalie rilevate per var_int
                    ...
                    for anomaly_date in anomalies_table_var["Date"]:
                        if anomaly_date not in anm_table_overall["date"].values:
                            new_row = {"date": anomaly_date}
                            for col in anm_table_overall.columns:
                                if col != "date":
                                    new_row[col] = 0
                            anm_table_overall = pd.concat([anm_table_overall, pd.DataFrame([new_row])],
                                                          ignore_index=True)
        ...
        # Una volta completata l'elaborazione per tutti gli elementi, inserisci la tabella nel dizionario
        key_name = f"anm_table_ctx{context}_cst{num_cluster}"
        anomalies_dict[key_name] = anm_table_overall.copy()

# Esempio: salvare ogni dataframe in CSV (o utilizzare il dizionario per ulteriori elaborazioni)
for key, df in anomalies_dict.items():
    df.to_csv(f"data/diagnosis/anomalies_table_var/{key}.csv", index=False)
