...
start_readable_cmp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
print(f"Starting CMP analysis: {start_readable_cmp}")
run_cmp(case_study)
end_time_cmp = time.time()
end_readable_cmp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
print(f"CMP analisys finished: {end_readable_cmp}")
minutes_cmp, seconds_cmp = divmod((end_time_cmp - start_time_cmp), 60)
print(f"Execution time: {int(minutes_cmp)} min {int(seconds_cmp)} sec")
