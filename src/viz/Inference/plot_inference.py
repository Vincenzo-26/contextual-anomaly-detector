...
def plot_inference_result(case_study, date, context):
    """
    Crea e mostra un grafico html che visualizza i risultati di inferenza di una rete bayesiana per un caso di studio
    specifico, una data e un contesto. tramite una visualizzazione gerarchica degli stati dei nodi nella rete,
    rappresentando le probabilità di ciascun nodo con barre colorate. Ad ogni nodo foglia è associato un pallino
    colorato in base alla propria thermal sensitivity: (verde per thermal sensitive, altrimemnti rosso).

    Input:
    case_study : str
        Nome del caso di studio.
    date : str
        Data specifica per cui visualizzare i risultati dell'inferenza.
    context : int o str
...
if __name__ == "__main__":
    plot_inference_result(case_study="Total", date="2024-07-17", context=2)
